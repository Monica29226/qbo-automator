...
async function fetchEmailsViaIMAP(
  ...
  try {
    console.log(`[Outlook-IMAP] Connecting to ${host}:${port}...`);
    ...
    console.log("[Outlook-IMAP] Greeting:", greeting.substring(0, 100));
    ...
    console.log("[Outlook-IMAP] Login response:", loginResp.substring(0, 100));
    ...
    console.log("[Outlook-IMAP] SELECT response:", selectResp.substring(0, 200));

    // Extract message count
    const existsMatch = selectResp.match(/\* (\d+) EXISTS/);
    const totalMessages = existsMatch ? parseInt(existsMatch[1]) : 0;
    console.log(`[Outlook-IMAP] INBOX has ${totalMessages} messages`);
    ...
    if (!searchLine || searchLine.trim() === "* SEARCH") {
      console.log("[Outlook-IMAP] No messages found");
      await sendCommand("A999", "LOGOUT");
      conn.close();
    ...
    console.log(`[Outlook-IMAP] Found ${messageIds.length} messages in date range`);
    ...
    console.log(`[Outlook-IMAP] Processing messages ${startIdx + 1} to ${startIdx + messagesToFetch.length} of ${messageIds.length} total (batch size: ${BATCH_SIZE})`);
    ...
    for (let i = 0; i < messagesToFetch.length; i++) {
      ...
      if (elapsedMs > MAX_EXECUTION_TIME_MS) {
        console.log(`[Outlook-IMAP] ⚠️ Approaching timeout (${elapsedMs}ms), stopping after ${i} messages`);
        break;
      }
      ...
      try {
        ...
        while (Date.now() < maxFetchTime && fetchAttempts < 200) {
          ...
          if (Date.now() - functionStartTime > MAX_EXECUTION_TIME_MS) {
            console.log(`[Outlook-IMAP] ⚠️ Global timeout during message fetch, stopping`);
            break;
          }
        }
        ...
        console.error(`[Outlook-IMAP] Error fetching message ${msgId}:`, msgErr);
      }
    }
    ...
    console.error("[Outlook-IMAP] Connection error:", error);
  ...
  for (const part of parts) {
    ...
    if (encoding === "BASE64") {
      try {
        ...
        console.error(`[Outlook-IMAP] Error decoding base64 for ${filename}:`, e);
        continue;
      }
    ...
    if (isHaciendaResponse(filename, decodedContent)) {
      console.log(`[Outlook-IMAP] Skipping Hacienda response/message: ${filename}`);
      continue;
    }

    if (decodedContent.includes("<Clave>")) {
      console.log(`[Outlook-IMAP] ✓ Valid invoice XML found: ${filename}`);
      attachments.push({ filename, content: decodedContent });
    } else {
      console.log(`[Outlook-IMAP] Skipping XML without Clave: ${filename}`);
    }
  }
  ...
  for (const part of parts) {
    ...
    if (encoding === "BASE64") {
      try {
        ...
        console.error(`[Outlook-IMAP] Error decoding PDF base64 for ${filename}:`, e);
        continue;
      }
    } else {
      const encoder = new TextEncoder();
    ...
    if (decodedContent.length > 4) {
      ...
      if (header === "%PDF") {
        console.log(`[Outlook-IMAP] ✓ Valid PDF found: ${filename} (${decodedContent.length} bytes)`);
        attachments.push({ filename, content: decodedContent });
      } else {
        console.log(`[Outlook-IMAP] Skipping invalid PDF (wrong header): ${filename}`);
      }
    }
  }
  ...
  for (const xml of xmlAttachments) {
    ...
    for (const pdf of pdfAttachments) {
      ...
      if (pdfBaseName === xmlBaseName ||
        ...
        matchedPdf = pdf;
        console.log(`[Outlook-IMAP] ✓ Matched PDF ${pdf.filename} to XML ${xml.filename}`);
        break;
      }
    }
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`[Outlook-IMAP] Fetching invoices for organization ${organization_id}${skip_count ? ` (skipping first ${skip_count} messages)` : ''}`);
    ...
    if (accountError || !hostingerAccount) {
      return new Response(
        JSON.stringify({
          ...
          message: "Microsoft 365 IMAP no está conectado. Ve a Integraciones y conecta/reconecta tu cuenta.",
        }),
        {
    ...
    if (!credentials?.email || !credentials?.password) {
      return new Response(
        JSON.stringify({
          ...
          message: "La conexión de Microsoft 365 IMAP está incompleta. Por favor reconecta la cuenta.",
        }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
          status: 200,
    ...
    const imapHost = credentials.imap_host || "outlook.office365.com";
    const imapPort = credentials.imap_port || 993;
    
    console.log(`[Outlook-IMAP] Connecting to ${imapHost}:${imapPort} for ${credentials.email}`);
    ...
    console.log(`[Outlook-IMAP] Searching emails since ${sinceDateStr}${beforeDateStr ? ` before ${beforeDateStr}` : ''}`);
    ...
    if (imapError) {
      console.error("[Outlook-IMAP] IMAP error:", imapError);
      ...
      if (isAuthFailed) {
        ...
        try {
          ...
          if (deactivateError) console.error("[Outlook-IMAP] Failed to deactivate integration account:", deactivateError);
        } catch (e) {
          console.error("[Outlook-IMAP] Failed to deactivate integration account (exception):", e);
        }

        try {
          const { error: orgUpdateError } = await supabase
            .from("organizations")
            .update({ outlook_connected: false, updated_at: now })
            .eq("id", organization_id);
          if (orgUpdateError) console.error("[Outlook-IMAP] Failed to update organization outlook_connected:", orgUpdateError);
        } catch (e) {
          console.error("[Outlook-IMAP] Failed to update organization outlook_connected (exception):", e);
        }
        ...
        return new Response(
          JSON.stringify({
            ...
            message:
              "No se pudo autenticar en el correo Microsoft 365 IMAP. Verifica la contraseña del buzón (no la del panel) y, si tienes 2FA, usa una contraseña de aplicación. Luego reconecta Hostinger.",
          }),
          {
            headers: { ...corsHeaders, "Content-Type": "application/json" },
            status: 200,
    ...
    console.log(`[Outlook-IMAP] Fetched ${rawEmails.length} raw emails with potential attachments`);
    ...
    for (const rawEmail of rawEmails) {
      ...
      if (Date.now() - processingStartTime > MAX_PROCESSING_TIME_MS) {
        console.log(`[Outlook-IMAP] ⚠️ Approaching timeout during processing, stopping early`);
        stoppedEarly = true;
        break;
      }
      ...
      try {
        ...
        for (const xml of xmlAttachments) {
          try {
            ...
            const claveMatch = xml.content.match(/<Clave>(\d{50})<\/Clave>/);
            if (!claveMatch) {
              console.log(`[Outlook-IMAP] No Clave found in ${xml.filename}, skipping`);
              continue;
            }
            ...
            if (xmlUploadError) {
              console.error(`[Outlook-IMAP] Error uploading XML ${clave}:`, xmlUploadError);
            }
            ...
            if (matchedPdf) {
              ...
              if (pdfUploadError) {
                console.error(`[Outlook-IMAP] Error uploading PDF ${clave}:`, pdfUploadError);
              } else {
                pdfUrl = pdfPath;
              }
            }
            ...
            console.log(`[Outlook-IMAP] 📤 Sending to process-document-xml: clave=${clave}, pdf_attachment_url=${pdfUrl || 'NONE'}`);
            ...
            console.error(`[Outlook-IMAP] Error processing XML ${xml.filename}:`, xmlErr);
            errors.push(`${xml.filename}: ${xmlErr}`);
            invoicesFailed++;
          }
        }
      } catch (emailErr) {
        console.error("[Outlook-IMAP] Error processing email:", emailErr);
        invoicesFailed++;
      }
    }
    ...
    console.log(`[Outlook-IMAP] Completed: ${invoicesProcessed} processed, ${invoicesFailed} failed. Total messages: ${totalFound || 'unknown'}, processed this run: ${processedCount || 'unknown'}, next_skip: ${nextSkip}${stoppedEarly ? ' (stopped early due to timeout)' : ''}`);
    ...
    console.error("[Outlook-IMAP] Error:", error);
