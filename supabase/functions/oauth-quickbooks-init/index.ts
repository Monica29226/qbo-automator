...
serve(async (req) => {
  ...
  try {
    ...
    if (credError || !credentials) {
      ...
      return new Response(
        JSON.stringify({ error: "No se encontraron credenciales de QuickBooks para esta organización" }),
        { status: 404, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }

    // Construir URL de autorización de QuickBooks
    ...
    console.error("Error in oauth-quickbooks-init:", error);
    return new Response(
