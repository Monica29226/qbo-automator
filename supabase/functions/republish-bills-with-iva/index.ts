...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`Starting bills with IVA republish for organization: ${organization_id}`);
    ...
    if (!bills || bills.length === 0) {
      return new Response(
        JSON.stringify({ 
          success: true, 
          message: "No se encontraron facturas con IVA para republicar", 
          processed: 0 
        }),
    ...
    console.log(`Found ${bills.length} bills with IVA to republish`);
    ...
    for (const doc of bills) {
      try {
        console.log(`Processing bill: ${doc.doc_number} with QBO ID: ${doc.qbo_entity_id}`);
        ...
        console.error(`Error processing bill ${doc.doc_number}:`, error);
        results.failed++;
        results.errors.push({
    ...
    // Paso 3: Invocar publish-to-quickbooks para republicar con IVA dividido correctamente
    console.log(`Invoking publish-to-quickbooks to republish bills with correct IVA...`);
    ...
    try {
      ...
      if (publishError) {
        ...
        console.log(`✓ Republished ${results.republished} bills with correct IVA`);
      }
    } catch (publishError) {
    ...
    const summary = {
      ...
      message: `Procesadas ${results.processed} facturas: ${results.deleted} eliminadas, ${results.republished} republicadas con IVA corregido, ${results.failed} fallidas`,
    };

    console.log("Bills with IVA republish completed:", summary);

    return new Response(JSON.stringify(summary), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (error) {
    console.error("Error in republish-bills-with-iva:", error);
    return new Response(
      JSON.stringify({
        error: error instanceof Error ? error.message : "Unknown error",
      }),
