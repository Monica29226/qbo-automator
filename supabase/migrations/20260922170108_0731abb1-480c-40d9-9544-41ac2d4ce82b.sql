DROP FUNCTION IF EXISTS public.compras_formato_gti(date, date, uuid);

CREATE OR REPLACE FUNCTION public.compras_formato_gti(p_desde date, p_hasta date, p_org uuid DEFAULT NULL::uuid, p_solo_publicados boolean DEFAULT false)
 RETURNS TABLE(organization_id uuid, empresa text, cedula_empresa text, doc_key text, consecutivo text, issue_date date, cedula_emisor text, nombre_emisor text, tipo text, moneda text, tipo_cambio numeric, t1 numeric, t2 numeric, t4 numeric, t8 numeric, t13 numeric, iva_total numeric, base_gravada numeric, descuento numeric, otros_cargos numeric, total_comprobante numeric, exento numeric, revisar boolean, status text, qbo_entity_id text)
 LANGUAGE sql
...
WITH b AS (
  SELECT d.id, d.organization_id, d.doc_key, d.doc_type, d.issue_date, d.xml_data,
    d.status AS st, d.qbo_entity_id AS qid,
  ...
  WHERE d.issue_date >= p_desde AND d.issue_date < p_hasta
    ...
    AND (NOT p_solo_publicados OR (d.status = 'published' AND d.qbo_entity_id IS NOT NULL))
    AND (public.has_role(auth.uid(),'admin') OR public.is_organization_member(auth.uid(), d.organization_id))
), l AS (
...
SELECT b.organization_id, o.name AS empresa, o.tax_id AS cedula_empresa,
  ...
  b.st AS status, b.qid AS qbo_entity_id
FROM b JOIN organizations o ON o.id=b.organization_id
LEFT JOIN a ON a.id=b.id
ORDER BY o.name, b.issue_date, consecutivo;
$function$;

GRANT EXECUTE ON FUNCTION public.compras_formato_gti(date, date, uuid, boolean) TO authenticated;