CREATE OR REPLACE FUNCTION public.compras_formato_gti(p_desde date, p_hasta date, p_org uuid DEFAULT NULL)
RETURNS TABLE(organization_id uuid, empresa text, cedula_empresa text, doc_key text, consecutivo text, issue_date date, cedula_emisor text, nombre_emisor text, tipo text, moneda text, tipo_cambio numeric, t1 numeric, t2 numeric, t4 numeric, t8 numeric, t13 numeric, iva_total numeric, base_gravada numeric, descuento numeric, otros_cargos numeric, total_comprobante numeric, exento numeric, revisar boolean)
LANGUAGE sql
STABLE
SECURITY DEFINER
...
REVOKE ALL ON FUNCTION public.compras_formato_gti(date, date, uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.compras_formato_gti(date, date, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.compras_formato_gti(date, date, uuid) TO service_role;