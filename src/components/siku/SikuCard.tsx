...
export function SikuCard({ organizationId }: Props) {
  ...
  const handleSave = async () => {
    if (!organizationId || !apiKey) {
      toast.error("Ingrese su API Key de Siku");
      return;
    }
    setSaving(true);
    try {
  ...
  const maskedKey = account?.credentials?.api_key
    ? `••••${account.credentials.api_key.slice(-4)}`
    : "—";
          ...
          <div className="flex-1">
            ...
            {loading ? (
              ...
              <div className="space-y-3 max-w-md">
                <div>
                  <Label htmlFor="siku-key">API Key de Siku</Label>
                  <Input
                    id="siku-key"
                    type="password"
                    value={apiKey}
                ...
                <div>
                  <Label htmlFor="siku-url">URL Base (opcional)</Label>
                  <Input
                    id="siku-url"
                    value={baseUrl}
